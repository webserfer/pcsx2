...
#define CFG "cfg"
		...
		static void button_clicked_wheel(GtkWidget* widget, gpointer data)
		{
		...
		int GtkPadConfigure(int port, const char* dev_type, const char* apititle, const char* apiname, GtkWindow* parent, ApiCallbacks& apicbs)
		{
			...
			// Remapping
			{
				...
				for (int i = 0; i < (int)countof(button_labels); i++)
				{
					...
					g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(button_clicked_wheel), reinterpret_cast<gpointer>(port));
		...
		int GtkKeyboardmaniaConfigure(int port, const char* dev_type, const char* apititle, const char* apiname, GtkWindow* parent, ApiCallbacks& apicbs)
		{
			...
			// Remapping
			{
				...
				for (auto& key : keys)
				{
					...
					g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(button_clicked), reinterpret_cast<gpointer>(port));
					g_object_set_data(G_OBJECT(button), JOYTYPE, reinterpret_cast<gpointer>(key.index));
					g_object_set_data(G_OBJECT(button), CFG, &cfg);
				...
				frame = gtk_frame_new("Buttons");
				{
					...
					button = gtk_button_new_with_label("Start (22)");
					g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(button_clicked), reinterpret_cast<gpointer>(port));
					g_object_set_data(G_OBJECT(button), JOYTYPE, reinterpret_cast<gpointer>(22));
					g_object_set_data(G_OBJECT(button), CFG, &cfg);
					gtk_box_pack_start(GTK_BOX(frame_box), button, FALSE, FALSE, 5);

					button = gtk_button_new_with_label("Select (14)");
					g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(button_clicked), reinterpret_cast<gpointer>(port));
					g_object_set_data(G_OBJECT(button), JOYTYPE, reinterpret_cast<gpointer>(14));
					g_object_set_data(G_OBJECT(button), CFG, &cfg);
					gtk_box_pack_start(GTK_BOX(frame_box), button, FALSE, FALSE, 5);
				}
				...
				frame = gtk_frame_new("Wheel");
				{
					...
					button = gtk_button_new_with_label("Up (29)");
					g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(button_clicked), reinterpret_cast<gpointer>(port));
					g_object_set_data(G_OBJECT(button), JOYTYPE, reinterpret_cast<gpointer>(29));
					g_object_set_data(G_OBJECT(button), CFG, &cfg);
					gtk_box_pack_start(GTK_BOX(frame_box), button, FALSE, FALSE, 5);

					button = gtk_button_new_with_label("Down (30)");
					g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(button_clicked), reinterpret_cast<gpointer>(port));
					g_object_set_data(G_OBJECT(button), JOYTYPE, reinterpret_cast<gpointer>(30));
					g_object_set_data(G_OBJECT(button), CFG, &cfg);
					gtk_box_pack_start(GTK_BOX(frame_box), button, FALSE, FALSE, 5);
				}
			}
